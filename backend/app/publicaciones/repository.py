    ...
    with _connect() as connection:
        connection.execute(
            ...
            CREATE TABLE IF NOT EXISTS publicaciones (
                ...
                modalidad TEXT NOT NULL CHECK (modalidad IN ('venta', 'alquiler')),
...
def create_publication(data: dict) -> dict:
    ...
    with _connect() as connection:
        cursor = connection.execute(
            """
            INSERT INTO publicaciones (titulo, descripcion, precio, modalidad, estado)
            VALUES (?, ?, ?, ?, ?)
            """,
            (
                data["titulo"],
        ...
        row = connection.execute(
            """
            SELECT id, titulo, descripcion, precio, modalidad, estado
            FROM publicaciones
            WHERE id = ?
            """,
            (publication_id,),
