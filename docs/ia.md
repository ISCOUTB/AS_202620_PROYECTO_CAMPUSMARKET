...
| Herramienta | Uso realizado | Verificación del equipo | Qué se rechazó y por qué |
|---|---|---|---|
| ChatGPT | Apoyo para analizar la pasada temprana del revisor automático de S7 e identificar la diferencia entre artefactos realmente ausentes y evidencia existente que no había podido ser verificada por el agente. | El equipo contrastó cada observación con `contracts/openapi-v1.json`, `backend/app/main.py`, `backend/app/publicaciones/router.py`, `backend/tests/test_contrato_openapi.py`, `.github/workflows/backend-tests.yml`, los runs de GitHub Actions, `docs/aspectos.md` y `docs/ia.md`. | Se rechazó modificar la arquitectura, inventar nuevos endpoints o alterar el contrato únicamente para elevar el resultado del revisor, porque los cuatro criterios específicos marcados como `No verificado` ya tenían implementación real y requerían principalmente evidencia auditable. |
| ChatGPT | Apoyo para sanear `docs/evidencias/evidencia-s7-2026-09-15.md`, haciendo explícitos `components.schemas`, el cotejo bidireccional contrato ↔ código, el historial Git del contrato, la línea del workflow que ejecuta la prueba contractual y los runs verde y rojo. | Se comprobó que el contrato declara OpenAPI `3.1.0` y API `1.0.0`; que `POST /publicaciones`, `GET /publicaciones` y `GET /health` corresponden con FastAPI; que el contrato se incorporó en `485249a`; que el Run #93 terminó en `success`; y que existe una ejecución roja real ante un cambio incompatible. | Se rechazó marcar evidencias como verificadas basándose únicamente en prosa. Se utilizaron rutas, commits y URLs reproducibles del repositorio. |
| ChatGPT | Apoyo para agregar al `README.md` un índice auditable de S7 que facilite al revisor localizar directamente contrato, schemas, implementación, prueba contractual, workflow, runs, aspectos, IA, ADR, arc42 y C4. | El equipo comprobó que los enlaces apuntan a artefactos existentes y que el README distingue entre evidencia específica S7 ya disponible y el pendiente transversal de SonarQube Cloud. | Se rechazó ocultar o declarar resuelto SonarQube Cloud mientras el workflow no invoque explícitamente el scanner, porque `CONTRATO.md` exige scanner en CI, run exitoso y URL pública con Quality Gate. |
| ChatGPT | Apoyo para evaluar el pendiente transversal de SonarQube Cloud sin restaurar de forma automática la configuración manual eliminada anteriormente. | Se verificó que el proyecto oficial `ISCOUTB_AS_202620_PROYECTO_CAMPUSMARKET` posee Quality Gate aprobado y análisis público, pero que `.github/workflows/backend-tests.yml` todavía no invoca el scanner de SonarQube Cloud. | Se rechazó volver a introducir sin validación el antiguo scanner asociado a una configuración personal o duplicar Automatic Analysis y CI-based analysis, porque podría generar una configuración inconsistente con el proyecto oficial del curso. |
